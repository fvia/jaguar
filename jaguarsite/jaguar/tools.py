...
def register_Archives():
    """ register all not registered files as Archives, in the JAGUAR_FILES folder
    """
    files_on_folder = [f for f in os.listdir(JAGUAR_FILES) if os.path.isfile( os.path.join( JAGUAR_FILES ,f ) )]
...
def create_Links():
    """ create symbolics links on JAGUAR_LINKS, for every Link that not has an associated symbolic link.
    """
...
def delete_Links():
    ...
    """
    links_on_folder = [l for l in os.listdir(JAGUAR_LINKS) if os.path.islink( os.path.join( JAGUAR_LINKS ,l ) )]
